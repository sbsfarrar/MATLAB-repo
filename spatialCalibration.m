function spatialCalibration()
% This function allows you to spatially calibrate your image and then make distance or area measurements
% that will be passed into the image analysis app.
...
button = menu('Choose Image Magnification:', '4X', '10X', 'Exit');
switch button
	case 1
		% Get the name of the file that the user wants to use.
		defaultFileName = fullfile(cd, '*.*');
		[baseFileName, folder] = uigetfile(defaultFileName, 'Select a 4X image file with scale bar');
		if baseFileName == 0
	...
	case 2
		% Get the name of the file that the user wants to use.
		defaultFileName = fullfile(cd, '*.*');
		[baseFileName, folder] = uigetfile(defaultFileName, 'Select a 10X image file with scale bar');
		if baseFileName == 0
			% User clicked the Cancel button.
			return;
		end
...
try
    ...
    assignin('base','distancePerPixel',calibration.distancePerPixel);
	success = true;
